...
//start off as identity - late we will init it with proper values.
	...
	void GLES2Lesson::render(IntGameMap map, IntGameMap actors, IntGameMap splats, LightMap lightMap, IntField ids, AnimationList movingCharacters, long animationTime) {
		...
		for (int z = 0; z < 50; ++z) {
			for (int x = 0; x < 50; ++x) {
				int tile = map[49 - z ][ x ];
				int actor = actors[ 49 - z][ x ];
				int splatFrame = splats[ 49 - z ][ x ];
				bool isCursorPoint = ( ( x == static_cast<int>(this->cursorPosition.x) ) && ( ( 49 - z ) == static_cast<int>(this->cursorPosition.y)) );

				float shade = ( 0.25f * std::min( 255, lightMap[49 - z ][ x ] ) / 255.0f ) + 0.75f;
				...
				if (id != 0) {
					...
					if ( id != 0 && movingCharacters.count( id ) > 0 ) {
						...
						fz = 49.0f - (( step * ( destPosition.y - (prevPosition.y) ) ) + prevPosition.y );
					}
					...
					if ( splatFrame > -1 ) {
						glBindTexture(GL_TEXTURE_2D, mTextures[ splatFrame + ETextures::Explosion  ]->mTextureId );
						drawGeometry(vboBillboardVertexDataIndex,
						             vboBillboardVertexIndicesIndex,
						             6,
						             getCubeTransform(glm::vec3(-10 + (fx * 2), -4.0f, -10 + (-fz * 2)))
