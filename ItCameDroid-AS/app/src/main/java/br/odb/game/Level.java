...
public class Level extends TileArea {
	...
	public Level(int i, int j, Resources resources, int[] tilePaletteIndexes, int chancesTile, int[] wallTiles, int chancesWall) {
		super(i, j, br.odb.game.Constants.BASETERRAINRESID, resources,
...
//		}
	...
	public boolean mayMoveTo(int x, int y) {
		...
		if (x < 0 || y < 0 || x >= br.odb.game.Constants.SIZEX
				|| y >= br.odb.game.Constants.SIZEY)
			return false;

		return super.getTileAt(x, y).getTipo() != br.odb.game.Constants.WALLRESID;
	}
